...
module.exports.run = async (miya, message) => {
    ...
    if(args[2] != "공지" && args[2] != "입퇴장" && args[2] != "로그") return error.wrongcmd(message, "`미야야 채널설정 [공지/입퇴장/로그] #채널멘션`")
    if(args[3] === undefined || args[3].indexOf("<#") === -1) return error.wrongcmd(message, "`미야야 채널설정 [공지/입퇴장/로그] #채널멘션`")
...
exports.helps = {
    description: '로그, 입퇴장, 공지 채널을 설정합니다.',
    uses: '채널설정',
